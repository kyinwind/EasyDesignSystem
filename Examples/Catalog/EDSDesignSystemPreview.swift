...
private struct EDSSystemColorWell: NSViewRepresentable {
    ...
    class Coordinator: NSObject {
        var parent: EDSSystemColorWell
        init(_ parent: EDSSystemColorWell) { self.parent = parent }
...
public struct EDSDesignSystemPreview: View {
    ...
    @State private var draftControlSize: EDSControlSizeTokens = EDSTheme.shared.controlSize
    @State private var draftHeroGradient: EDSHeroGradient = EDSTheme.shared.heroGradient

    @State private var selectedPreset: EDSPresetTheme? = nil
    ...
    private var presetPicker: some View {
        Picker("预设", selection: $selectedPreset) {
            Text("（当前）").tag(nil as EDSPresetTheme?)
            ForEach(EDSPresetTheme.allPresets) { preset in
                Text(preset.name).tag(preset as EDSPresetTheme?)
            }
        }
    ...
    private var previewPanel: some View {
        ScrollView {
            VStack(spacing: EDSTheme.shared.spacing.xl) {
                // 颜色预览
                previewSection("颜色") {
    ...
    private func colorRow(_ label: String, color: Binding<Color>) -> some View {
        HStack(spacing: 8) {
            ...
            EDSSystemColorWell(color: color)
...
// MARK: - EDSThemeJSONDocument：用于文件导出的 Document 类型

public struct EDSThemeJSONDocument: FileDocument {
