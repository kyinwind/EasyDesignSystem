...
public enum EDSColorHexFormat: Sendable {
...
extension Color {
    ...
    public init(hex: String, format: EDSColorHexFormat) {
        self.init(hex: hex, format: format, allowsEightDigitHex: true)
    }

    private init(hex: String, format: EDSColorHexFormat, allowsEightDigitHex: Bool) {
...
// MARK: - EDSSpacingTokens

public struct EDSSpacingTokens: Codable, Equatable, Sendable {
...
// MARK: - EDSStrokeTokens

public struct EDSStrokeTokens: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        hairline = try container.decodeValue(CGFloat.self, forKey: .hairline, default: EDSStrokeTokens().hairline)
    }
}
// MARK: - EDSShadowTokens

public struct EDSShadowTokens: Codable, Equatable, Sendable {
    /// 阴影颜色（hex 字符串）
    ...
    /// 默认卡片阴影：微弱、大范围
    public static let card = EDSShadowTokens(
        color: "#000000", opacity: 0.06, radius: 18, x: 0, y: 10
    )

    /// 轻量阴影：更淡更小
    public static let subtle = EDSShadowTokens(
        color: "#000000", opacity: 0.04, radius: 8, x: 0, y: 2
    )

    /// 强调阴影：更深更明显（用于浮层）
    public static let prominent = EDSShadowTokens(
        color: "#000000", opacity: 0.15, radius: 24, x: 0, y: 16
    )
}

public struct EDSRadiusTokens: Codable, Equatable, Sendable {
...
// MARK: - EDSTypographyTokens（只存数值，运行时生成 Font）

public struct EDSTypographyTokens: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let defaults = EDSTypographyTokens()
...
// MARK: - EDSControlSizeTokens

public struct EDSControlSizeTokens: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let defaults = EDSControlSizeTokens()
...
public struct EDSHeroGradient: Codable, Equatable, Sendable {
...
// MARK: - EDSDesignTokens：完整设计 Token 集合

public struct EDSDesignTokens: Codable, Equatable, Sendable {
    public var colors: EDSColorTokens = EDSColorTokens()
    public var spacing: EDSSpacingTokens = EDSSpacingTokens()
    ...
    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        colors = try container.decodeValue(EDSColorTokens.self, forKey: .colors, default: EDSColorTokens())
        spacing = try container.decodeValue(EDSSpacingTokens.self, forKey: .spacing, default: EDSSpacingTokens())
    ...
    public static let heroGradientBlue = EDSHeroGradient(
        startColor: Color(hexRGB: "#3185FF"),
        endColor:   Color(hexRGB: "#0A6BFF")
    )

    public static let heroGradientOrange = EDSHeroGradient(
        startColor: Color(hexRGB: "#FF6B00"),
        endColor:   Color(hexRGB: "#FF3D00")
    )

    public static let heroGradientPurple = EDSHeroGradient(
