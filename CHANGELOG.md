...
- 新增 `EDSCommandGroup`：为同一任务域中的相邻命令提供统一的背景、描边、间距与圆角，适合桌面端密集工具栏。
- 新增 `EDSSplitButton`：保留一键主操作，并通过独立下拉段收纳同类替代命令；主操作与菜单支持分别禁用。
- 新增 `EDSCommandPopover`：使用明确的命令域名称打开结构化浮层，适合页面操作、插入和格式设置等可扩展命令集合。
- Catalog 新增“命令组合”示例，README 增加三个组件的组合用法。

### Compatibility

- 仅新增公开 API，没有删除或改变既有组件及初始化方法。
- 最低平台保持 iOS 17 / macOS 14，不新增第三方依赖。
- 公共 API 基线已更新，单元测试共 27 项通过。
...
- **`filled` 彩色档文字统一白色**：`success` / `warning` 实心档从 `textPrimary` 改为
  ...
  白字在 #27B15A / #F9B135 上对比度 2.8:1 / 1.9:1，不达 WCAG AA——杨哥目视定版取舍。
- **`medium` 底色 50% → 25%**（`outline` 废弃别名同步）。
- **`medium` 文字改为"深一档的同色系"**：`tone` 色 RGB×0.7（`neutral` 维持 `textPrimary`），
  色彩身份贯穿 filled/medium/soft/plain 四档，且与 soft 拉开文字深浅层次。
